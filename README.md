...
No. R3bring.Client will capture automatically all windows (existed or a new one) by its extended titles (format is 'handle - title - size'). 

Captured windows will be transferred to the R3bring.Server according connection properties. 

R3bring.Client pull all mouse\keyboard events back from the R3bring.Server to operate remotely with any captured window.

## Is it secure?
It's depend on you. It's look like a browser (web client) make a HTTP(s) session to somewhere (web server) with some packed traffic inside. 
...
2. Set connectionMode to Sercure into server.json and client.json

## I have a hopper. Is it possible to interact with the Client and ask it to capture only the necessary windows by handles?
Short answer is Yes. But you have to put these handles to client.json config when you need to add or update it.

Set FilterPattern value by your hopper in the format (handle1)|(handle2)|... and R3bring.Client will automatically reload the config to use the new one windows filter pattern.

## I have the Super mouse button pressing code. Can I use it with the Client?
Yes. You can just implement header functions below, compile your C++ code to Operator.dll and put it into Dll folder (just replace the original one). 

It has to be x64 format dynamic link library. R3bring.Client on mouse event from R3bring.Server will call activate_window and then one of the mouse_click function, same behavior for the keyboard event.
...
## I need to use the Old Windows for OH in my Virtual Machine. What are you support here?
...
Select appropriate one IP, set host value to it into server.json as well as you set it into client.json (or from R3bring.Client GUI).
...
Yes. Set R3BringClientMode value into client.json to Agent mode - at starting it will load config and start capturing according its parameters.
...
Press Ctrl+C into Console or just close Console window.

R3bring.Client GUI File->Close menu button just close the GUI, to exit you have to press File->Exit.

## Do I can to use many R3Bring.Clients with one R3Bring.Server?
No. And we can't imagine scenario why you need it for now. 

But if you want you can run different pairs of the server and client to use different port by each pair.

## I try to create the best filter for the different windows. How do I cat to create powerful pattern for all of this?
...
## Is it consume all my resources to capture it all?
It use your GPU for capturing.

It use your RAM to keep bitmaps. (more captured windows - more consumed memory)

It use your CPU to optimize and pack the data to transfer it by network from Client to Server.

We didn't expect too much resource consuming (depends on your machine config) and suppose it can be ~5-7%.
